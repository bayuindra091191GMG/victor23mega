...
<div class="container">
    ...
    {{ Form::open(['route'=>['admin.material_requests.report.download'],'method' => 'post','id' => 'general-form']) }}
    ...
    <table class="table" style="font-size: 10px;">
        ...
        <tbody>
        @foreach($mrHeaders as $item)
            <tr>
                <td colspan="4"><b>{{ $item->code }} - {{ $item->date_string }} - {{ $item->department->name }} {{ !empty($item->machinery->code) ? '- '. $item->machinery->code : '' }} - Status: {{ $item->status->description }}</b></td>
            </tr>
            @foreach($item->material_request_details as $detail)
                <tr>
                    <td class="text-left">{{ $detail->item->code }}</td>
                    <td class="text-left">{{ str_limit($detail->item->name, 15) }}</td>
                    <td class="text-left">{{ $detail->item->part_number ?? '-' }}</td>
