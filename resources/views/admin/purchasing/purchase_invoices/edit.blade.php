...
@section('title','Ubah Purchase Invoice '. $header->code)

@section('content')
    <div class="row" style="margin-bottom: 10px;">
        <div class="col-md-12 col-sm-12 col-xs-12">

            {{ Form::open(['route'=>['admin.purchase_invoices.update', $header->id],'method' => 'put','id' => 'general-form','class'=>'form-horizontal form-label-left']) }}
            ...
            <div class="form-group">
                ...
                <div class="col-md-6 col-sm-6 col-xs-12">
                    <input type="text" id="code" name="code" class="form-control col-md-12 col-xs-12" value="{{ $header->code }}" readonly/>
                </div>
            </div>
            ...
            <div class="form-group">
                ...
                <div class="col-md-6 col-sm-6 col-xs-12">
                    <input type="text" id="po_code" name="po_code" class="form-control col-md-12 col-xs-12" value="{{ $header->purchase_order_header->code }}" readonly/>
                </div>
            </div>
            ...
            <div class="form-group">
                <div class="col-md-6 col-sm-6 col-xs-12 col-md-offset-3">
                    <a class="btn btn-danger" href="{{ route('admin.purchase_invoices.show', ['purchase_invoice' => $header->id]) }}"> Batal</a>
    ...
    <script type="text/javascript">
        ...
        {{--$('#po_code').select2({--}}
            {{--placeholder: {--}}
                ...
                {{--text: '{{ $header->purchase_order_header->code }}'--}}
            {{--},--}}
            {{--width: '100%',--}}
        ...
        $('.modal-footer').on('click', '.add', function() {
            $.ajax({
                type: 'POST',
                url: '{{ route('admin.purchase_invoice_details.store') }}',
                data: {
                    '_token': $('input[name=_token]').val(),
                    'header_id': '{{ $header->id }}',
                    'item': $('#item_add').val(),
        ...
        $('.modal-footer').on('click', '.edit', function() {
            $.ajax({
                type: 'PUT',
                url: '{{ route('admin.purchase_invoice_details.update') }}',
                data: {
                    '_token': $('input[name=_token]').val(),
                    'id' : id,
                    'item': $("#item_edit").val(),
        ...
        $('.modal-footer').on('click', '.delete', function() {
            $.ajax({
                type: 'POST',
                url: '{{ route('admin.purchase_invoice_details.delete') }}',
                data: {
                    '_token': $('input[name=_token]').val(),
                    'id': deletedId,
                    'header_id': '{{ $header->id }}'
