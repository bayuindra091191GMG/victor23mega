        ...
        <div class="col-md-12 col-sm-12 col-xs-12">
            ...
            <div class="form-group">
                ...
                <div class="col-md-6 col-sm-6 col-xs-12">
                    <input id="date" type="text" class="form-control col-md-7 col-xs-12 @if($errors->has('date')) parsley-error @endif"
                           name="date" value="{{ $header->date_string }}" required>
                </div>
            </div>
            ...
            <div class="form-group">
                <div class="col-md-6 col-sm-6 col-xs-12 col-md-offset-3">
                    <a class="btn btn-danger" href="{{ route('admin.quotations.show', ['quotation' => $header->id]) }}"> Batal</a>
...
@section('scripts')
    ...
    <script type="text/javascript">
        ...
        $('#supplier').select2({
            placeholder: {
                id: '{{ $header->supplier_id ?? '-1' }}',
                text: '{{ $header->supplier->name ?? '- Pilih Vendor -' }}'
            },
            width: '100%',
        ...
        $('.modal-footer').on('click', '.add', function() {
            $.ajax({
                type: 'POST',
                url: '{{ route('admin.quotation_details.store') }}',
                data: {
                    '_token': $('input[name=_token]').val(),
                    'header_id': '{{ $header->id }}',
                    'item': $('#item_add').val(),
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if ((data.errors)) {
                        ...
                        $('.item' + data.id).replaceWith("<tr class='item" + data.id + "'><td class='text-center'>" + data.item.code + " - " + data.item.name + "</td><td class='text-center'>" + data.item.uom + "</td><td class='text-center'>" + data.quantity + "</td><td class='text-right'>" + data.price_string + "</td><td class='text-right'>" + data.discount_amount_string + "</td><td class='text-right'>" + data.subtotal_string + "</td><td>" + remarkEdit + "</td><td class='text-center'>" + "<button class='edit-modal btn btn-info' data-id='" + data.id + "' data-item-id='" + data.item_id + "' data-item-text='" + data.item.code + " " + data.item.name + "' data-qty='" + data.quantity + "' data-remark='" + data.remark + "' data-price='" + data.price + "' data-discount='" + data.discount + "'><span class='glyphicon glyphicon-edit'></span></button><button class='delete-modal btn btn-danger' data-id='" + data.id + "' data-item-id='" + data.item_id + "' data-item-text='" + data.item.code + " - "  + data.item.name + "' data-qty='" + data.quantity + "' data-price='" + data.price + "' data-discount='" + data.discount + "'><span class='glyphicon glyphicon-trash'></span></button></td></tr>");
                    }
                }
            });
        });
